...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!projects || projects.length === 0) {
      return NextResponse.json({ projects: [], messages: [], members: [] });
    }
    ...
    const members = await getProjectMembersDB(String(currentProjectId));
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await deleteProjectMessageDB(Number(messageId), session.id);
