...
export default function ConversationsPage() {
  ...
  const fetchConversations = useCallback(async (isInitial = false) => {
    try {
      ...
      if (selectedProjectId) params.set('projectId', selectedProjectId.toString());
      ...
      if (data.projects && Array.isArray(data.projects)) {
        ...
        if (!selectedProjectId && data.projects.length > 0) {
          setSelectedProjectId(data.selectedProjectId || data.projects[0].id);
        }
      }
  ...
  }, [selectedProjectId, activeChannel]);
              ...
              <button
                onClick={() => {
                  if (selectedProject?.key) {
                    navigator.clipboard.writeText(selectedProject.key);
                    toast.success(`Copied Project Key: ${selectedProject.key}`);
                  }
                }}
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#18191D] hover:bg-[#202126] text-[#CFD4DD] hover:text-white border border-[#2B2D33] text-xs font-mono transition-colors"
                title="Copy Project Key"
              >
                <Copy size={12} />
                <span>{selectedProject?.key || 'PRJ'}</span>
              </button>
            </div>
          </div>
